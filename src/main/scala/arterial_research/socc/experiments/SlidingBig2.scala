 ...
 * Window much shorter.
 */
trait SlidingBig2Setup {
  val base_name = "/scratch/tjhunter_tmp_watson/logs/"
  val exp_name = "sliding_big_2"
  ...
  val test_config = new TestConfig {
    ...
    states_directory = test_base_name + "/states/SlidingBig2/"
    test_output_directory = test_base_name + "/perf/SlidingBig2/"
...
object SlidingBig2 extends App with MMLogging with SlidingBig2Setup {
...
object SlidingBig2Test extends App with MMLogging with SlidingBig2Setup {
  // We are going to run the streaming algorithm in batch mode
  // We will schedule a streaming call at some fixed intervals
  // Each interval will be decided beforehand.
