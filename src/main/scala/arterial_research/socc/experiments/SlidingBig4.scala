 ...
 * Window much shorter.
 */
trait SlidingBig4Setup {
  val base_name = "/scratch/tjhunter_tmp_watson/logs/"
  val exp_name = "sliding_big_4"
  ...
  val test_config = new TestConfig {
    ...
    states_directory = test_base_name + "/states/SlidingBig4/"
    test_output_directory = test_base_name + "/perf/SlidingBig4/"
...
object SlidingBig4 extends App with MMLogging with SlidingBig4Setup {
...
object SlidingBig4Test extends App with MMLogging with SlidingBig4Setup {
  // We are going to run the streaming algorithm in batch mode
  // We will schedule a streaming call at some fixed intervals
  // Each interval will be decided beforehand.
